...
const ChatInfo = () => {
  ...
  return (
    <motion.div
      ...
      animate={{ width: 'var(--width)', opacity: 100 }}
      exit={{ width: 0, opacity: 0 }}
      transition={{ duration: 0.3 }}
      className="absolute right-0 z-100 h-dvh border-l border-border bg-background [--width:100%] md:static md:[--width:35%]"
    >
      {showChatInfo === 'Chat Info' && (
        <div className="z-100 h-full w-full overflow-y-scroll scrollbar-none">
          <div className="sticky top-0 z-10 flex h-14 items-center gap-4 bg-background p-2">
            <span
              className="cursor-pointer"
              onClick={() => toggleShowChat(null)}
            >
              <X />
            </span>
          ...
          <div className="relative z-0 flex flex-col items-center gap-2">
            <motion.div
              ...
              onClick={() =>
                setImagePreview(
                  isGroup ? groupAvatar?.url : friend?.avatar?.url
                )
              }
              className="relative"
            >
